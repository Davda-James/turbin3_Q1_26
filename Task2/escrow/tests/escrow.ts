...
describe("Escrow", () => {
  ...
  before(async () => {
    ...
    const takerAtaATx = new anchor.web3.Transaction().add(
      createAssociatedTokenAccountInstruction(taker.publicKey, takerAtaA, taker.publicKey, mintA)
    );
    await provider.sendAndConfirm(takerAtaATx, [taker]);
    
    makerAtaB = getAssociatedTokenAddressSync(mintB, maker);
    const makerAtaBTx = new anchor.web3.Transaction().add(
      createAssociatedTokenAccountInstruction(maker, makerAtaB, maker, mintB)
    );
    await provider.sendAndConfirm(makerAtaBTx);
  });
  ...
  it("Makes and refunds the escrow", async () => {
    ...
    await program.methods
      .doMake(seed1, new anchor.BN(depositAmount), new anchor.BN(receiveAmount))
      .accountsStrict({
        maker: maker,
        mintA: mintA,
        mintB: mintB,
  ...
  it("Makes and takes the escrow", async () => {
    ...
    await program.methods
      .doMake(seed2, new anchor.BN(depositAmount), new anchor.BN(receiveAmount))
      .accountsStrict({
        maker: maker,
        mintA: mintA,
        mintB: mintB,
  ...
  const ixClose = await program.methods
  .doTakeClose()
  .accountsStrict({
    maker: maker,
    escrow: escrowPda,
  ...
  const tx = new anchor.web3.Transaction().add(ixDeposit, ixWithdraw, ixClose);
