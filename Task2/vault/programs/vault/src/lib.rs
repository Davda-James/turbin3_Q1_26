...
declare_id!("AizHmamwqLJh8vUUXbBchgPYsstUrYPsXxRVBYmHdL7M");
...
impl<'info> Withdraw<'info> {
    pub fn withdraw(&mut self, _amount: u64) -> Result<()> {
        ...
        require!(_amount <= self.vault.lamports() - rent_exempt, ErrorMsg::InsufficientBalance);

        let cpi_accounts = Transfer {
            from: self.vault.to_account_info(),
            to: self.user.to_account_info(),
        };
        let vault_state_bytes = self.vault_state.key();

        let seeds = &[b"vault",vault_state_bytes.as_ref(), &[self.vault_state.vault_bump]];
...
impl<'info> Close<'info> {
    pub fn close(&mut self) -> Result<()> {
        ...
        let vault_state_bytes = self.vault_state.key();
        let signer_seeds: &[&[&[u8]]] = &[&[b"vault",vault_state_bytes.as_ref(), &[self.vault_state.vault_bump]]];
